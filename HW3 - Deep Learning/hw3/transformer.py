...
def sliding_window_attention(q, k, v, window_size, padding_mask=None):
    ...
    # ====== YOUR CODE: ======
    def get_query_indices(window_size=window_size, seq_len=seq_len):
        query_indices = []
        for i in range(window_size // 2):
            query_indices.append(list(i for _ in range(window_size // 2 + 1 + i)))
        for i in range(window_size // 2, seq_len - window_size // 2):
            query_indices.append(list(i for _ in range(window_size + 1)))
        for i in (range(window_size // 2)):
            query_indices.append(list((seq_len - window_size // 2 + i) for _ in range(window_size - i)))
        return np.concatenate(query_indices)

    def get_key_indices(window_size=window_size, seq_len=seq_len):
        key_indices = []
        for i in range(window_size // 2):
            key_indices.append(list(j for j in range(window_size // 2 + 1 + i)))
        for i in range(window_size // 2, seq_len - window_size // 2):
            key_indices.append(list(j for j in range(i - window_size // 2, i + window_size // 2 + 1)))
        for i in (range(window_size // 2)):
            key_indices.append(list((seq_len - window_size + j + i) for j in range(window_size - i)))
        return np.concatenate(key_indices)

    def get_attention_tensor(q=q, k=k, batch_size=batch_size, seq_len=seq_len, query_indices=None, key_indices=None):
        attention = None
        if q.dim() > 3:
        ...
        attention[:, ..., query_indices, key_indices] = torch.einsum(
            '...ij,...jk->...ik',
            q[:, ..., query_indices, :].unsqueeze(-2),
            k[:, ..., key_indices, :].unsqueeze(-1)
        ).squeeze(-1).squeeze(-1)
        return attention

    attention = get_attention_tensor(query_indices=get_query_indices(), key_indices=get_key_indices())
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, padding_mask):
        ...
        # ====== YOUR CODE: ======
        res_1 = x + self.dropout(self.self_attn(x, padding_mask))
        norm_1 = self.norm1(res_1)
        res_2 = norm_1 + self.dropout(self.feed_forward(norm_1))
        x = self.norm2(res_2)
        # ========================
        return x
...
class Encoder(nn.Module):
    ...
    def forward(self, sentence, padding_mask):
        ...
        encoded = torch.cat([layer(encoded, padding_mask) for layer in self.encoder_layers], dim=1)
        output = self.classification_mlp(encoded[:, 0]).squeeze(-1)
        # ========================
        return output  
