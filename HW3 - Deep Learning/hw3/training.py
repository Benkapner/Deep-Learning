...
def freeze_model_except_for_the_last_2_linear_layers(model: nn.Module) -> None:
    ...
    lin_layers = [module for module in model.modules() if isinstance(module, nn.Linear)]
    for layer in lin_layers[-2:]:
...
class TransformerEncoderTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        loss.backward()
        self.optimizer.step()
        num_correct = (torch.round(torch.sigmoid(predictions)) == label).sum()
        # ========================
