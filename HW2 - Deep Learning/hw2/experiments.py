...
def mlp_experiment(
    ...
    model = BinaryClassifier(
        model=MLP(in_dim=len(dl_train.dataset.tensors[0][0]), dims=[*[width] * depth, 2],
...
def cnn_experiment(
    ...
    **kw,
    ...
    fit_res = None
    # ====== YOUR CODE: ======
    channels, conv_pars, pooling = [filters for filters in filters_per_layer for i in range(layers_per_block)], {
        'kernel_size': 3, 'padding': 1, 'stride': 1}, {'kernel_size': 2}
    shape = ds_train[0][0].shape
    model = ArgMaxClassifier(
        model_cls(shape, 10, channels, pool_every, hidden_dims, conv_params=conv_pars, pooling_params=pooling))
    loss = torch.nn.CrossEntropyLoss()
    opt = torch.optim.Adam(model.parameters(), lr=lr)
    cls_trainer = ClassifierTrainer(model, loss_fn=loss, optimizer=opt, device=device)
    test_dataloader = torch.utils.data.DataLoader(ds_test, batch_size=bs_test, shuffle=False, num_workers=2)
    train_dataloader = torch.utils.data.DataLoader(ds_train, batch_size=bs_train, shuffle=True, num_workers=2)

    fit_res = cls_trainer.fit(dl_train=train_dataloader, dl_test=test_dataloader, num_epochs=epochs,
