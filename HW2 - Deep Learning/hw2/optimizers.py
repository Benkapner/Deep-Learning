...
class MomentumSGD(Optimizer):
    ...
    def step(self):
        for idx, (p, dp) in enumerate(self.params):
            ...
            # ====== YOUR CODE: ======
            self.v[idx] = self.momentum * self.v[idx] - self.learn_rate * (dp + self.reg * p)
            p += self.v[idx]
            # ========================
...
class RMSProp(Optimizer):
    ...
    def step(self):
        for idx, (p, dp) in enumerate(self.params):
            ...
            # parameters tensor.
            # ====== YOUR CODE: ======
            self.r[idx] = self.decay * self.r[idx] + (1 - self.decay) * torch.square(dp + self.reg * p)
            p -= (self.learn_rate / torch.sqrt(self.r[idx] + self.eps)) * (dp + self.reg * p)
            # ========================
