...
class LeakyReLU(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        out = torch.maximum(x,self.alpha*x)
        # ========================
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        dx = torch.where((x>0),1.,self.alpha)*dout
        # ========================
...
class Sigmoid(Layer):
    ...
    def backward(self, dout):
        ...
        # TODO: Implement gradient w.r.t. the input x
        # ====== YOUR CODE: ======
        exp = torch.exp(-self.grad_cache["x"])
        dx = exp/(torch.square(1+exp))*dout
        # ========================
...
class TanH(Layer):
    ...
    def backward(self, dout):
        ...
        exp_pos = torch.exp(x)
        exp_neg = torch.exp(-x)
        tanh_squared = torch.square((exp_pos-exp_neg)/(exp_pos+exp_neg))
        dx = (1-tanh_squared)*dout
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        dx = torch.exp(x)/torch.reshape(torch.sum(torch.exp(x), dim=1),(-1,1))
        dx[range(N),y] -= 1
        dx *= dout/N
        # ========================
...
class Dropout(Layer):
    ...
    def forward(self, x, **kw):
        ...
        if self.training_mode:
            dropout = torch.bernoulli(torch.full_like(x,1-self.p))
            out = x*dropout
...
class MLP(Layer):
    ...
    def __init__(
        ...
        **kw,
        ...
        # ====== YOUR CODE: ======
        for i, d in enumerate(hidden_features):
            layers.append(Linear(in_features, d))
            if activation == "relu":
                layers.append(ReLU())
            else:
