...
def plot_decision_boundary_2d(
    ...
    x1_grid, x2_grid = torch.meshgrid(
        torch.arange(torch.min(x[:, 0]), torch.max(x[:, 0]), dx),
        torch.arange(torch.min(x[:, 1]), torch.max(x[:, 1]), dx)
    )
...
def select_roc_thresh(
    ...
    y_score_pos_class = y_score[:, classifier.positive_class]
    fpr, tpr, thresholds = roc_curve(y, y_score_pos_class, pos_label=classifier.positive_class)
    dists_from_opt = torch.from_numpy((1 - tpr) ** 2 + (fpr - 0) ** 2)  # dist from (0,1)

    optimal_thresh_idx = torch.argmin(dists_from_opt).item()
