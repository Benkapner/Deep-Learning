...
class Trainer(abc.ABC):
    ...
    def fit(
        ...
        for epoch in range(num_epochs):
            ...
            train_loss += [torch.mean(torch.FloatTensor(train_result.losses)).item()]
            test_loss += [torch.mean(torch.FloatTensor(test_result.losses)).item()]
...
class ClassifierTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        num_correct = torch.sum((y == torch.argmax(res, dim=1)).int()).item()
...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        self.optimizer.zero_grad()
        res = self.model.forward(X.flatten().reshape(X.shape[0], -1))
        loss = self.loss_fn(res, y)
        num_correct = torch.sum((y == torch.argmax(res, dim=1)).int()).item()
