...
class CNN(nn.Module):
    ...
    def _make_feature_extractor(self):
        ...
        for i in range(len(self.channels)):
            layers.append(
                nn.Conv2d(in_channels=input_output_channels[i],
                          out_channels=input_output_channels[i + 1],
                          **self.conv_params)
            )
    ...
    def _make_mlp(self):
        ...
        nonlins = [ACTIVATIONS[self.activation_type](**self.activation_params) for _ in \
                   range(len(self.hidden_dims))]
...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        # ====== YOUR CODE: ======
        main_path = []
        input_output_channels = [in_channels] + channels
        for i in range(len(input_output_channels) - 1):
            ...
            if dropout > 0 and i < len(input_output_channels) - 2:
                main_path.append(nn.Dropout2d(dropout))
            if batchnorm and i < len(input_output_channels) - 2:
                main_path.append(nn.BatchNorm2d(input_output_channels[i + 1]))
            if i < len(input_output_channels) - 2:
                main_path.append(ACTIVATIONS[activation_type](**activation_params))
        self.main_path = nn.Sequential(*main_path)

        shortcut_path = [ACTIVATIONS[None]()]
        if input_output_channels[0] != input_output_channels[-1]:
            shortcut_path.append(
                nn.Conv2d(
                    input_output_channels[0],
    ...
    def forward(self, x: Tensor):
        ...
        short_out = self.shortcut_path(x)
...
class ResNet(CNN):
    ...
    def _make_feature_extractor(self):
        ...
        # ====== YOUR CODE: ======
        block_start_idx = 0
        while block_start_idx < len(self.channels):
            block_end_idx = min(block_start_idx + self.pool_every, len(self.channels))
            block_channels = self.channels[block_start_idx:block_end_idx]
            ...
            ) if self.bottleneck and in_channels == block_channels[0] else ResidualBlock(
            ...
            layers.append(block)
            if (block_end_idx - block_start_idx) == self.pool_every:
                layers.append(POOLINGS[self.pooling_type](**self.pooling_params))
            in_channels = block_channels[-1]
            block_start_idx += self.pool_every
        # ========================
        seq = nn.Sequential(*layers)
        return seq
